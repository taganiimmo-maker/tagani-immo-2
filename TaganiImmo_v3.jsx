...
/* ═══════════════════════════════════════════════════
   TAGANI IMMO v3 — ADDITIONAL FEATURE MODULES
   All new features from conversations added below
═══════════════════════════════════════════════════ */

/* ─── FIREBASE EMAIL/PASSWORD AUTH ─── */
// Replace anonymous auth with full email/password system
// Add these imports to the top of App.jsx:
// import { createUserWithEmailAndPassword, signInWithEmailAndPassword,
//   sendPasswordResetEmail, updatePassword, onAuthStateChanged } from "firebase/auth"

/* ─── USER SELF-REGISTRATION ─── */
export function RegisterScreen({onBack,dark}){
  const[name,setName]=useState("");const[email,setEmail]=useState("");
  const[pwd,setPwd]=useState("");const[pin,setPin]=useState("");const[err,setErr]=useState("");
  const Th=makeS(dark);
  const doReg=async()=>{
    if(!name.trim()){setErr("Entrez votre nom.");return;}
    if(pwd.length<8){setErr("Mot de passe: minimum 8 caractères.");return;}
    try{
      const settings=await get(ref(db,"settings"));
      const teamPin=settings.exists()?settings.val().teamPin:"1234";
      if(pin!==teamPin){setErr("PIN d'équipe incorrect. Contactez l'administrateur.");return;}
      const cred=await createUserWithEmailAndPassword(auth,email,pwd);
      await set(ref(db,`users/${cred.user.uid}`),{uid:cred.user.uid,name:name.trim(),email,role:"agent",status:"pending",createdAt:Date.now()});
    }catch(e){setErr(e.code==="auth/email-already-in-use"?"Email déjà utilisé.":e.message);}
  };
  const inpS={width:"100%",padding:"12px 14px",borderRadius:8,border:`1px solid ${Th.border}`,background:dark?"#0d1117":Th.bg,color:Th.text,fontSize:15,marginBottom:8,fontFamily:"inherit",minHeight:48};
  return(
    <div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",background:Th.bg,padding:"60px 22px 30px",fontFamily:"system-ui,sans-serif"}}>
      <button onClick={onBack} style={{background:"none",border:"none",color:GOLD,cursor:"pointer",fontSize:22,marginBottom:20}}>← Retour</button>
      <div style={{color:GOLD,fontWeight:700,fontSize:20,letterSpacing:2,marginBottom:6}}>CRÉER MON COMPTE</div>
      <div style={{color:Th.text2,fontSize:13,marginBottom:24}}>Votre compte sera activé par l'administrateur.</div>
      <input value={name} onChange={e=>setName(e.target.value)} placeholder="Nom complet *" style={inpS}/>
      <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email *" style={inpS}/>
      <input type="password" value={pwd} onChange={e=>setPwd(e.target.value)} placeholder="Mot de passe (min 8 car.) *" style={inpS}/>
      <input type="password" inputMode="numeric" maxLength={4} value={pin} onChange={e=>setPin(e.target.value)} placeholder="Code PIN d'équipe *" style={{...inpS,letterSpacing:6,textAlign:"center",fontSize:20}}/>
      {err&&<div style={{color:Th.error,fontSize:13,padding:"8px 12px",background:Th.error+"12",borderRadius:6,marginBottom:10}}>{err}</div>}
      <button onClick={doReg} style={{width:"100%",padding:14,borderRadius:8,background:GOLD,color:"#0d1117",border:"none",cursor:"pointer",fontSize:16,fontWeight:700,minHeight:52}}>Créer mon compte →</button>
    </div>
  );
}

/* ─── CAMERA MEASUREMENT WITH YELLOW OVERLAY ─── */
export function CameraMeasure({onClose,gps,dark}){
  const vRef=useRef(null);const cRef=useRef(null);
  const[stream,setStream]=useState(null);const[captured,setCaptured]=useState(null);const[dims,setDims]=useState(null);
  useEffect(()=>{
    navigator.mediaDevices?.getUserMedia({video:{facingMode:"environment",width:{ideal:1920},height:{ideal:1080}}})
      .then(s=>{setStream(s);if(vRef.current){vRef.current.srcObject=s;vRef.current.play();}})
      .catch(()=>{alert("Caméra non disponible.");onClose(null);});
    return()=>{stream?.getTracks().forEach(t=>t.stop());};
  },[]);
  useEffect(()=>{
    if(!stream)return;
    const iv=setInterval(()=>{
      const c=cRef.current,v=vRef.current;if(!c||!v||!v.videoWidth)return;
      c.width=v.videoWidth;c.height=v.videoHeight;
      const ctx=c.getContext("2d"),W=c.width,H=c.height,pad=50;
      ctx.clearRect(0,0,W,H);
      /* Yellow measurement frame */
      ctx.strokeStyle="#C9A84C";ctx.lineWidth=5;ctx.setLineDash([18,10]);
      ctx.strokeRect(pad,pad,W-pad*2,H-pad*2);ctx.setLineDash([]);
      /* Yellow corner markers */
      const cs=40;[[pad,pad],[W-pad,pad],[pad,H-pad],[W-pad,H-pad]].forEach(([x,y])=>{
        const sx=x<W/2?1:-1,sy=y<H/2?1:-1;
        ctx.strokeStyle="#C9A84C";ctx.lineWidth=7;
        ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x+sx*cs,y);ctx.moveTo(x,y);ctx.lineTo(x,y+sy*cs);ctx.stroke();
      });
      /* Yellow header */
      const fs=Math.max(18,W*0.04);ctx.fillStyle="rgba(0,0,0,0.7)";ctx.fillRect(0,0,W,fs+24);
      ctx.font=`bold ${fs}px monospace`;ctx.fillStyle="#C9A84C";ctx.textAlign="center";ctx.textBaseline="top";
      ctx.fillText("📐 MESURE EN COURS — TAGANI IMMO",W/2,8);
      /* Yellow measurements */
      const w_m=`~${(W*0.035).toFixed(1)}m`,h_m=`~${(H*0.035).toFixed(1)}m`,a_m=`~${(parseFloat(w_m)*parseFloat(h_m)).toFixed(0)}m²`;
      ctx.font=`bold ${fs}px monospace`;ctx.fillStyle="rgba(0,0,0,0.6)";ctx.fillRect(W/2-120,H-pad-26,240,32);
      ctx.fillStyle="#C9A84C";ctx.textAlign="center";ctx.textBaseline="middle";
      ctx.fillText(`${w_m} × ${h_m} ≈ ${a_m}`,W/2,H-pad-10);
      /* GPS stamp in yellow */
      if(gps){ctx.font=`${Math.max(11,fs*0.5)}px monospace`;ctx.fillStyle="rgba(0,0,0,0.5)";ctx.fillRect(0,H-22,W,22);ctx.fillStyle="#C9A84C";ctx.textAlign="center";ctx.fillText(`GPS: ${gps.lat.toFixed(5)}, ${gps.lng.toFixed(5)}`,W/2,H-8);}
      setDims({w:w_m,h:h_m,area:a_m});
    },100);
    return()=>clearInterval(iv);
  },[stream,gps]);
  const capture=()=>{
    const v=vRef.current;const oc=document.createElement("canvas");
    oc.width=v.videoWidth;oc.height=v.videoHeight;
    const ctx=oc.getContext("2d");ctx.drawImage(v,0,0);
    if(cRef.current)ctx.drawImage(cRef.current,0,0);
    stream?.getTracks().forEach(t=>t.stop());
    setCaptured(oc.toDataURL("image/jpeg",0.92));
  };
  const download=()=>{const a=document.createElement("a");a.href=captured;a.download=`tagani_mesure_${Date.now()}.jpg`;a.click();};
  return(
    <div style={{position:"fixed",inset:0,zIndex:9999,background:"#000",display:"flex",flexDirection:"column"}}>
      {!captured?(<>
        <div style={{flex:1,position:"relative"}}>
          <video ref={vRef} autoPlay playsInline muted style={{width:"100%",height:"100%",objectFit:"cover"}}/>
          <canvas ref={cRef} style={{position:"absolute",inset:0,width:"100%",height:"100%",pointerEvents:"none"}}/>
          {dims&&<div style={{position:"absolute",bottom:80,left:0,right:0,textAlign:"center",color:"#C9A84C",fontSize:18,fontWeight:700,fontFamily:"monospace",textShadow:"0 2px 8px #000"}}>{dims.area}</div>}
        </div>
        <div style={{display:"flex",gap:12,padding:16,background:"#111"}}>
          <button onClick={()=>{stream?.getTracks().forEach(t=>t.stop());onClose(null);}} style={{flex:1,padding:15,borderRadius:10,background:"#333",color:"#fff",border:"none",cursor:"pointer",fontSize:16}}>✕ Fermer</button>
          <button onClick={capture} style={{flex:2,padding:15,borderRadius:10,background:"#C9A84C",color:"#0d1117",border:"none",cursor:"pointer",fontSize:16,fontWeight:700}}>📸 Capturer</button>
        </div>
      </>):(<>
        <img src={captured} style={{flex:1,objectFit:"contain"}} alt="Mesure terrain"/>
        <div style={{display:"flex",gap:10,padding:16,background:"#111"}}>
          <button onClick={()=>setCaptured(null)} style={{flex:1,padding:14,borderRadius:10,background:"#333",color:"#fff",border:"none",cursor:"pointer",fontSize:15}}>↩ Reprendre</button>
          <button onClick={download} style={{flex:1,padding:14,borderRadius:10,background:"#27ae60",color:"#fff",border:"none",cursor:"pointer",fontSize:15,fontWeight:700}}>📥 Télécharger</button>
          <button onClick={()=>onClose(captured)} style={{flex:1,padding:14,borderRadius:10,background:"#C9A84C",color:"#0d1117",border:"none",cursor:"pointer",fontSize:15,fontWeight:700}}>✓ Utiliser</button>
        </div>
      </>)}
    </div>
  );
...
/* ─── VOICE RECORDER ─── */
export function VoiceRecorder({onSend,dark}){
  const[recording,setRecording]=useState(false);const[mr,setMr]=useState(null);const[dur,setDur]=useState(0);const timerRef=useRef(null);
  const Th=makeS(dark);
  const start=async()=>{
    try{const stream=await navigator.mediaDevices.getUserMedia({audio:true});const chunks=[];const rec=new MediaRecorder(stream,{mimeType:"audio/webm"});rec.ondataavailable=e=>chunks.push(e.data);rec.onstop=()=>{const blob=new Blob(chunks,{type:"audio/webm"});const url=URL.createObjectURL(blob);onSend({blob,url,duration:dur});stream.getTracks().forEach(t=>t.stop());};setMr(rec);rec.start();setRecording(true);setDur(0);timerRef.current=setInterval(()=>setDur(d=>d+1),1000);}catch(e){alert("Microphone non disponible. Vérifiez les permissions.");}
  };
  const stop=()=>{if(mr&&recording){mr.stop();setRecording(false);clearInterval(timerRef.current);}};
  const fmtD=d=>`${Math.floor(d/60)}:${String(d%60).padStart(2,"0")}`;
  return(
    <button onMouseDown={start} onMouseUp={stop} onTouchStart={e=>{e.preventDefault();start();}} onTouchEnd={e=>{e.preventDefault();stop();}}
      style={{padding:"12px 16px",borderRadius:8,border:`1.5px solid ${recording?"#e74c3c":Th.border}`,background:recording?"#e74c3c1a":"transparent",color:recording?"#e74c3c":Th.text2,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",gap:10,fontFamily:"inherit",minHeight:50,userSelect:"none",width:"100%",justifyContent:"center"}}>
      <span style={{fontSize:22}}>🎙</span>
      {recording?`⏹ ${fmtD(dur)} — Relâcher pour envoyer`:"Maintenir pour enregistrer un message vocal"}
    </button>
  );
}

/* ─── AUDIO PLAYER ─── */
export function AudioPlayer({url,duration,dark}){
  const[playing,setPlaying]=useState(false);const aRef=useRef(null);const Th=makeS(dark);
  const fmtD=d=>`${Math.floor(d/60)}:${String(d%60).padStart(2,"0")}`;
  const toggle=()=>{if(!aRef.current)return;playing?aRef.current.pause():aRef.current.play();setPlaying(!playing);};
  useEffect(()=>{if(aRef.current)aRef.current.onended=()=>setPlaying(false);},[]);
  return(<div style={{display:"flex",alignItems:"center",gap:10,padding:"4px 0"}}>
    <audio ref={aRef} src={url} style={{display:"none"}}/>
    <button onClick={toggle} style={{width:38,height:38,borderRadius:"50%",background:GOLD,color:"#0d1117",border:"none",cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>{playing?"⏸":"▶"}</button>
    <div style={{flex:1,height:5,background:Th.border,borderRadius:3,overflow:"hidden"}}><div style={{width:playing?"100%":"0%",height:"100%",background:GOLD,transition:playing?`width ${duration||10}s linear`:"none",borderRadius:3}}/></div>
    <span style={{color:Th.text2,fontSize:12,fontFamily:"monospace",minWidth:32}}>{fmtD(duration||0)}</span>
  </div>);
}

/* ─── PWA INSTALL PROMPT ─── */
export function InstallBanner({dark,onInstall,onDismiss}){
  const Th=makeS(dark);
  return(<div style={{background:dark?"#161b22":"#fff",border:`1px solid ${GOLD}44`,borderRadius:10,margin:"10px 14px",padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}>
    <div style={{fontSize:13,color:Th.text}}>📲 Installer Tagani Immo sur votre écran d'accueil</div>
    <div style={{display:"flex",gap:6,flexShrink:0}}>
      <button onClick={onInstall} style={{padding:"7px 12px",borderRadius:6,background:GOLD,color:"#0d1117",border:"none",cursor:"pointer",fontSize:12,fontWeight:700}}>Installer</button>
      <button onClick={onDismiss} style={{padding:"7px 10px",borderRadius:6,background:"transparent",color:Th.text2,border:`1px solid ${Th.border}`,cursor:"pointer",fontSize:12}}>×</button>
    </div>
  </div>);
}
...
export function OfflineBanner(){
  return(<div style={{background:"#8a6a00",color:"#fff",padding:"7px 14px",fontSize:12,textAlign:"center",fontWeight:500}}>📡 Hors ligne — vos données sont sauvegardées localement. Reconnectez-vous pour synchroniser.</div>);
}

/* ─── BOTTOM NAVIGATION BAR ─── */
export function BottomNav({tab,setTab,dark,unreadChat=0}){
  const Th=makeS(dark);
  const items=[{id:"home",icon:"🏠",label:"Accueil"},{id:"inspections",icon:"📋",label:"Inspections"},{id:"map",icon:"📍",label:"GPS"},{id:"chat",icon:"💬",label:"Chat"},{id:"profile",icon:"👤",label:"Profil"}];
  return(
    <nav style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:dark?"#161b22":"#fff",borderTop:`1px solid ${Th.border}`,display:"flex",justifyContent:"space-around",padding:"6px 0 env(safe-area-inset-bottom, 12px)",zIndex:100,boxShadow:"0 -2px 16px rgba(0,0,0,.15)"}}>
      {items.map(it=>(
        <button key={it.id} onClick={()=>setTab(it.id)} aria-label={it.label} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2,background:"transparent",border:"none",cursor:"pointer",padding:"4px 0 2px",position:"relative",minHeight:52}}>
          <span style={{fontSize:24}}>{it.icon}</span>
          <span style={{fontSize:10,color:tab===it.id?Th.gold:Th.text2,fontWeight:tab===it.id?700:400}}>{it.label}</span>
          {tab===it.id&&<div style={{position:"absolute",bottom:0,width:24,height:3,background:Th.gold,borderRadius:"2px 2px 0 0"}}/>}
          {it.id==="chat"&&unreadChat>0&&<div style={{position:"absolute",top:2,right:"18%",background:"#e74c3c",color:"#fff",fontSize:9,fontWeight:700,minWidth:16,height:16,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 3px"}}>{unreadChat}</div>}
        </button>
      ))}
    </nav>
  );
...
/* ─── GPS STATUS BAR ─── */
export function GPSStatusBar({gps,status,dark}){
  const col=status==="active"?"#27ae60":status==="loading"?"#e67e22":"#e74c3c";
  const bg=status==="active"?"#071a07":"#1a1200";
  const label=status==="active"?`● GPS · ${gps.lat.toFixed(5)}°N · ${gps.lng.toFixed(5)}°E · ±${gps.acc}m`:status==="loading"?"● GPS En cours de détection automatique...":"● GPS Indisponible — autorisez dans les paramètres de votre navigateur";
  return<div style={{background:bg,padding:"5px 14px",fontSize:11,color:col,fontFamily:"monospace",borderBottom:`1px solid ${col}22`}}>{label}</div>;
}

/* ─── PHOTO LIGHTBOX ─── */
export function PhotoLightbox({photos,startIndex,onClose,dark}){
  const[idx,setIdx]=useState(startIndex||0);
  const ph=photos[idx];
  const prev=()=>setIdx(i=>Math.max(0,i-1));
  const next=()=>setIdx(i=>Math.min(photos.length-1,i+1));
  useEffect(()=>{const h=e=>{if(e.key==="ArrowLeft")prev();if(e.key==="ArrowRight")next();if(e.key==="Escape")onClose();};window.addEventListener("keydown",h);return()=>window.removeEventListener("keydown",h);},[idx]);
  if(!ph)return null;
  const dlPhoto=()=>{const a=document.createElement("a");a.href=ph.url||ph.dataUrl;a.download=`tagani_photo_${idx+1}.jpg`;a.click();};
  return(
    <div style={{position:"fixed",inset:0,zIndex:9999,background:"rgba(0,0,0,0.95)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{display:"flex",flexDirection:"column",alignItems:"center",maxWidth:"100vw"}}>
        <img src={ph.url||ph.dataUrl} alt="" style={{maxWidth:"100vw",maxHeight:"78vh",objectFit:"contain"}} loading="eager"/>
        <div style={{padding:"8px 16px",color:"#ccc",fontSize:12,textAlign:"center",lineHeight:1.6}}>
          {ph.type&&<span style={{color:GOLD,marginRight:8}}>[{ph.type}]</span>}
          {ph.locationLabel&&<span>{ph.locationLabel} · </span>}
          {ph.loc&&<span style={{fontFamily:"monospace",fontSize:10}}>{ph.loc.lat?.toFixed(5)}, {ph.loc.lng?.toFixed(5)}</span>}
          {(ph.w||ph.sizeKB)&&<span style={{opacity:.6,marginLeft:8}}>{ph.w}×{ph.h} · {ph.sizeKB}KB</span>}
        </div>
        <div style={{display:"flex",gap:10,marginBottom:8}}>
          {idx>0&&<button onClick={prev} style={{padding:"10px 20px",borderRadius:8,background:"#333",color:"#fff",border:"none",cursor:"pointer",fontSize:18}}>‹</button>}
          <button onClick={dlPhoto} style={{padding:"10px 18px",borderRadius:8,background:"#27ae60",color:"#fff",border:"none",cursor:"pointer",fontSize:14,fontWeight:700}}>📥 Télécharger</button>
          <button onClick={onClose} style={{padding:"10px 18px",borderRadius:8,background:GOLD,color:"#0d1117",border:"none",cursor:"pointer",fontSize:14,fontWeight:700}}>✕ Fermer</button>
          {idx<photos.length-1&&<button onClick={next} style={{padding:"10px 20px",borderRadius:8,background:"#333",color:"#fff",border:"none",cursor:"pointer",fontSize:18}}>›</button>}
        </div>
        <div style={{color:"#666",fontSize:11}}>{idx+1} / {photos.length}</div>
      </div>
    </div>
  );
}
...
export function SkeletonCard({dark}){
  const Th=makeS(dark);
  return(
    <div style={{background:Th.bg3,borderRadius:14,padding:14,marginBottom:10,border:`1px solid ${Th.border}`}}>
      {[80,60,40].map((w,i)=>(
        <div key={i} style={{height:i===0?16:12,width:`${w}%`,background:dark?"#21262d":"#e8e8e8",borderRadius:6,marginBottom:i===2?0:8,animation:"pulse 1.5s ease infinite",animationDelay:`${i*0.1}s`}}/>
      ))}
    </div>
  );
}
...
export function UserCard({u,onApprove,onSuspend,onUnsuspend,onDelete,onChangePwd,dark,isSelf}){
  const Th=makeS(dark);
  const statusColors={active:"#27ae60",pending:"#e67e22",suspended:"#e74c3c"};
  const statusLabels={active:"● Actif",pending:"⏳ En attente",suspended:"⊘ Suspendu"};
  return(
    <div style={{background:Th.bg3,borderRadius:12,padding:"12px 14px",marginBottom:8,border:`1px solid ${Th.border}`}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div>
          <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:4}}>
            <div style={{width:34,height:34,borderRadius:"50%",background:GOLD+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:GOLD}}>{(u.name||"?")[0].toUpperCase()}</div>
            <div>
              <div style={{fontSize:14,fontWeight:600,color:Th.text}}>{u.name}</div>
              <div style={{fontSize:11,color:Th.text2}}>{u.email}</div>
            </div>
          </div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            <span style={{display:"inline-block",padding:"2px 8px",borderRadius:10,fontSize:10,fontWeight:700,background:GOLD+"22",color:GOLD,border:`1px solid ${GOLD}44`}}>{u.role}</span>
            <span style={{display:"inline-block",padding:"2px 8px",borderRadius:10,fontSize:10,fontWeight:700,background:statusColors[u.status]+"22",color:statusColors[u.status],border:`1px solid ${statusColors[u.status]}44`}}>{statusLabels[u.status]||u.status}</span>
          </div>
          {u.lastLogin&&<div style={{color:Th.text2,fontSize:10,marginTop:4}}>Dernière connexion: {fmt(u.lastLogin)}</div>}
        </div>
        {!isSelf&&<div style={{display:"flex",flexDirection:"column",gap:4}}>
          {u.status==="pending"&&<button onClick={()=>onApprove(u.uid)} style={{padding:"6px 12px",borderRadius:6,background:"#27ae601a",color:"#27ae60",border:"1px solid #27ae6044",cursor:"pointer",fontSize:11,fontWeight:700}}>✅ Approuver</button>}
          {u.status==="active"&&<button onClick={()=>onSuspend(u.uid)} style={{padding:"6px 12px",borderRadius:6,background:"#e67e221a",color:"#e67e22",border:"1px solid #e67e2244",cursor:"pointer",fontSize:11}}>⊘ Suspendre</button>}
          {u.status==="suspended"&&<button onClick={()=>onUnsuspend(u.uid)} style={{padding:"6px 12px",borderRadius:6,background:"#27ae601a",color:"#27ae60",border:"1px solid #27ae6044",cursor:"pointer",fontSize:11}}>▶ Réactiver</button>}
          <button onClick={()=>onChangePwd(u.uid)} style={{padding:"6px 12px",borderRadius:6,background:"transparent",color:Th.info,border:`1px solid ${Th.info}44`,cursor:"pointer",fontSize:11}}>🔑 Mot de passe</button>
          <button onClick={()=>onDelete(u.uid)} style={{padding:"6px 12px",borderRadius:6,background:"transparent",color:Th.error,border:`1px solid ${Th.error}44`,cursor:"pointer",fontSize:11}}>🗑 Supprimer</button>
        </div>}
      </div>
    </div>
  );
  ...
  if(view==="inspect"&&draft){
    const p=draft.property;
    const TABS=[{id:"details",label:t.tabs.details},{id:"owner",label:t.tabs.owner},{id:"photos",label:`${t.tabs.photos} (${photos.length})`},{id:"docs",label:t.tabs.docs},{id:"plan",label:t.tabs.plan},{id:"land",label:t.tabs.land},{id:"notes",label:t.tabs.notes}];
    const floorList=t.floors||FLOORS_FR||["RDC","1er","2ème","3ème","4ème"];
    return(
      <div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",background:S.bg,fontFamily:"system-ui,sans-serif"}}>
        <style>{CSS}</style>
        {showCameraMeasure&&<CameraMeasure gps={gps} onClose={captured=>{setShowCameraMeasure(false);if(captured){setPhotos(prev=>[...prev,{dataUrl:captured,ts:Date.now(),loc:gps?{lat:gps.lat,lng:gps.lng}:null,type:"Terrain",sizeKB:0,w:0,h:0,agentUid:fbUser.uid,agentName:userDoc?.displayName||""}]);setInspTab("photos");}}}/>}
        {/* Photo type modal */}
        {photoTypeModal!==null&&(
          <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"flex-end"}}>
            <div style={{width:"100%",maxWidth:430,margin:"0 auto",background:S.bg3,borderRadius:"16px 16px 0 0",padding:20}}>
              <div style={{color:S.text,fontWeight:600,fontSize:15,marginBottom:14}}>Type de photo</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,maxHeight:300,overflowY:"auto"}}>
                {t.photoTypes.map(type=><button key={type} onClick={()=>setPhotoType(photoTypeModal,type)} style={{padding:"10px 6px",borderRadius:8,border:`1px solid ${S.border}`,background:"transparent",color:S.text,fontSize:12,cursor:"pointer"}}>{type}</button>)}
              </div>
              <button onClick={()=>setPhotoTypeModal(null)} style={{...secondaryBtn(),width:"100%",marginTop:12}}>Ignorer</button>
            </div>
          </div>
        )}
        {/* Photo lightbox */}
        {showLightbox!==null&&(
          <div style={{position:"fixed",inset:0,zIndex:600,background:"rgba(0,0,0,0.95)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowLightbox(null)}>
            <img src={photos[showLightbox]?.dataUrl||photos[showLightbox]?.url} alt="" style={{maxWidth:"100%",maxHeight:"85vh",objectFit:"contain"}}/>
            <div style={{color:"#fff",fontSize:12,marginTop:10,textAlign:"center"}}>
              {photos[showLightbox]?.type||"Photo"} · {photos[showLightbox]?.sizeKB||0}KB
              {photos[showLightbox]?.loc&&<span> · {photos[showLightbox].loc.lat.toFixed(4)},{photos[showLightbox].loc.lng.toFixed(4)}</span>}
            </div>
            <div style={{display:"flex",gap:12,marginTop:12}}>
              <button onClick={e=>{e.stopPropagation();setShowLightbox(Math.max(0,showLightbox-1));}} style={{...secondaryBtn(),padding:"8px 16px"}}>←</button>
              <span style={{color:GOLD,fontSize:14,alignSelf:"center"}}>{showLightbox+1}/{photos.length}</span>
              <button onClick={e=>{e.stopPropagation();setShowLightbox(Math.min(photos.length-1,showLightbox+1));}} style={{...secondaryBtn(),padding:"8px 16px"}}>→</button>
            </div>
          </div>
        )}
        {/* Header */}
        <div style={{background:dark?"#0d1117":"#fff",borderBottom:`1px solid ${GOLD}33`,padding:"9px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:10}}>
          ...
          <div style={{textAlign:"center"}}>
            <div style={{color:GOLD,fontWeight:700,fontSize:11,letterSpacing:1.5}}>INSPECTION · {completionPct}%</div>
            {lastSaved&&<div style={{color:S.text2,fontSize:9}}>{t.autoSaved} {fmtTime(lastSaved)}</div>}
          </div>
          <div style={{display:"flex",gap:4}}>
            <button onClick={()=>saveInspection()} style={{padding:"6px 9px",borderRadius:7,border:`1px solid ${GOLD}44`,background:GOLD+"1a",color:GOLD,cursor:"pointer",fontSize:11,fontWeight:600}}>{t.save}</button>
            <button onClick={submitInspection} style={{padding:"6px 9px",borderRadius:7,border:"none",background:GOLD,color:"#0d1117",cursor:"pointer",fontSize:11,fontWeight:700}}>{t.submit}</button>
          </div>
        </div>
        {/* GPS banner */}
        {gps&&<div style={{background:dark?"#071a07":"#f0fff4",padding:"4px 14px",fontSize:10,display:"flex",gap:12,borderBottom:`1px solid ${gpsColor}33`}}><span style={{color:gpsColor}}>📍 {gps.lat.toFixed(5)}, {gps.lng.toFixed(5)}</span><span style={{color:S.text2}}>±{gps.acc}m</span></div>}
        {/* Progress bar */}
        <div style={{background:S.border,height:4}}><div style={{background:GOLD,height:4,width:`${completionPct}%`,transition:"width .4s"}}/></div>
        {/* Tab bar */}
        <div style={{display:"flex",gap:4,padding:"7px 10px",borderBottom:`1px solid ${S.border}`,overflowX:"auto"}}>
          {TABS.map(tt=><button key={tt.id} onClick={()=>setInspTab(tt.id)} style={tabBtn(inspTab===tt.id)}>{tt.label}</button>)}
        </div>
        <div style={{padding:14,paddingBottom:90,overflowY:"auto"}}>
          ...
          {inspTab==="details"&&<>
            <div style={card}>
              ...
              <input value={p.address} onChange={e=>updProp("address",e.target.value)} placeholder="Ex: 12 Rue des Acacias, Conakry" style={inp}/>
              <LocationSelector value={locVal} onChange={v=>{setLocVal(v);}} dark={dark} t={t}/>
            </div>
            <div style={card}>
              <label style={lbl}>{t.propType}</label>
              <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                {Object.entries(t.propTypes).map(([k,v])=><button key={k} onClick={()=>updProp("type",k)} style={{padding:"8px 12px",borderRadius:7,border:`1.5px solid ${p.type===k?S.gold:S.border}`,background:p.type===k?S.gold+"1a":"transparent",color:p.type===k?S.gold:S.text2,fontSize:13,cursor:"pointer"}}>{v}</button>)}
              </div>
            </div>
            <div style={card}>
              <label style={{...lbl,marginBottom:10}}>{t.rooms}</label>
              {Object.entries(t.rooms).map(([k,lbl2])=>(
                <div key={k} style={rowStyle}>
                  <span style={{fontSize:14}}>{lbl2}</span>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <button onClick={()=>updProp(k,Math.max(0,(p[k]||0)-1))} style={sbtn(false)}>−</button>
                    <span style={{color:GOLD,fontWeight:700,fontSize:17,minWidth:22,textAlign:"center"}}>{p[k]||0}</span>
                    <button onClick={()=>updProp(k,(p[k]||0)+1)} style={sbtn(true)}>+</button>
                  </div>
                </div>
              ))}
            </div>
            <div style={card}>
              <label style={lbl}>{t.measurements}</label>
              {[["totalArea","Surface habitable (m²)"],["landArea","Surface terrain (m²)"],["landHectares","Superficie (hectares)"],["yearBuilt","Année de construction"],["listPrice","Prix de vente (GNF)"]].map(([k,l2])=>(
                <div key={k} style={{marginBottom:10}}>
                  <label style={{...lbl,fontSize:9}}>{l2}</label>
                  <input type="number" value={p[k]||""} onChange={e=>updProp(k,e.target.value)} placeholder="0" style={inp}/>
                </div>
              ))}
            </div>
            <div style={card}>
              ...
              <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                {Object.entries(t.conditions).map(([k,v])=><button key={k} onClick={()=>updProp("condition",k)} style={{padding:"7px 12px",borderRadius:7,border:`1.5px solid ${p.condition===k?S.gold:S.border}`,background:p.condition===k?S.gold+"1a":"transparent",color:p.condition===k?S.gold:S.text2,fontSize:13,cursor:"pointer"}}>{v}</button>)}
              </div>
            </div>
            <div style={card}>
              <label style={lbl}>{t.feats}</label>
              <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                {t.features.map((f,i)=><button key={i} onClick={()=>toggleFeat(f)} style={{padding:"6px 10px",borderRadius:6,border:`1.5px solid ${feats.includes(f)?S.gold:S.border}`,background:feats.includes(f)?S.gold+"1a":"transparent",color:feats.includes(f)?S.gold:S.text2,fontSize:11,cursor:"pointer"}}>{f}</button>)}
              </div>
            </div>
            <div style={card}>
              <label style={lbl}>{t.agentNotes}</label>
              <textarea value={p.notes||""} onChange={e=>updProp("notes",e.target.value)} placeholder="Observations, état du quartier, travaux recommandés..." style={{...inp,minHeight:80,resize:"vertical"}}/>
            </div>
            <div style={{...card,border:`1px solid ${GOLD}22`}}>
              <label style={lbl}>📍 GPS</label>
              {gps?<>
                <div style={{display:"flex",gap:6,marginBottom:5}}>
                  {[["LAT",gps.lat.toFixed(7)],["LNG",gps.lng.toFixed(7)]].map(([l2,v])=><div key={l2} style={{flex:1,background:dark?"#0d1117":"#f6f8fa",borderRadius:8,padding:"8px",textAlign:"center"}}><div style={{color:S.text2,fontSize:9}}>{l2}</div><div style={{color:GOLD,fontSize:12,fontFamily:"monospace",fontWeight:700}}>{v}</div></div>)}
                </div>
                <a href={`https://maps.google.com/?q=${gps.lat},${gps.lng}`} target="_blank" rel="noreferrer" style={{color:S.info,fontSize:12}}>Ouvrir dans Google Maps →</a>
              </>:<div style={{color:S.text2,fontSize:13}}>{gpsLabel}</div>}
            </div>
          </>}
          ...
          {inspTab==="owner"&&<>
            <div style={card}>
              <label style={{...lbl,marginBottom:10}}>{t.ownerInfo}</label>
              {[["ownerName",t.ownerFields?.name||"Nom"],["ownerFirstName",t.ownerFields?.firstName||"Prénom(s)"],["ownerDOB",t.ownerFields?.dob||"Date de naissance","date"],["ownerPOB",t.ownerFields?.pob||"Lieu de naissance"],["ownerNationality",t.ownerFields?.nationality||"Nationalité"],["ownerOccupation",t.ownerFields?.occupation||"Profession"],["ownerPhone",t.ownerFields?.phone||"Téléphone","tel"],["ownerPhone2",t.ownerFields?.phone2||"Téléphone 2","tel"],["ownerEmail",t.ownerFields?.email||"Email","email"],["ownerAddress",t.ownerFields?.address||"Adresse"]].map(([k,l2,tp])=>(
                <div key={k} style={{marginBottom:10}}>
                  <label style={{...lbl,fontSize:9}}>{l2}</label>
                  <input type={tp||"text"} value={p[k]||""} onChange={e=>updProp(k,e.target.value)} placeholder={l2} style={inp}/>
                </div>
              ))}
              <label style={{...lbl,fontSize:9,marginTop:6}}>{t.ownerFields?.idType||"Type de pièce d'identité"}</label>
              <select value={p.ownerIdType||""} onChange={e=>updProp("ownerIdType",e.target.value)} style={{...inp}}>
                <option value="">— Sélectionnez —</option>
                {Object.entries(t.ownerIdTypes||{}).map(([k,v])=><option key={k} value={k}>{v}</option>)}
              </select>
              <label style={{...lbl,fontSize:9}}>{t.ownerFields?.idNum||"Numéro de pièce d'identité"}</label>
              <input value={p.ownerIdNum||""} onChange={e=>updProp("ownerIdNum",e.target.value)} placeholder="Numéro de la pièce d'identité" style={inp}/>
              <label style={{...lbl,fontSize:9}}>{t.ownerFields?.relation||"Relation à la propriété"}</label>
              <select value={p.ownerRelation||""} onChange={e=>updProp("ownerRelation",e.target.value)} style={inp}>
                <option value="">— Sélectionnez —</option>
                {Object.entries(t.ownerRelations||{}).map(([k,v])=><option key={k} value={k}>{v}</option>)}
              </select>
            </div>
            <div style={card}>
              <label style={lbl}>📸 Photo de la Pièce d'Identité</label>
              <p style={{color:S.text2,fontSize:12,marginBottom:10}}>Photographiez la CNI, passeport ou permis de conduire du propriétaire.</p>
              {ownerIdPh?(<div style={{position:"relative"}}><img src={ownerIdPh.dataUrl||ownerIdPh.url} alt="ID" style={{width:"100%",borderRadius:8,border:`1px solid ${S.border}`,display:"block"}}/><button onClick={()=>setOwnerIdPh(null)} style={{position:"absolute",top:6,right:6,background:"#c0392bcc",color:"#fff",border:"none",borderRadius:"50%",width:24,height:24,cursor:"pointer",fontSize:12}}>✕</button><div style={{color:S.text2,fontSize:10,marginTop:4}}>{fmt(ownerIdPh.ts)}</div></div>):(
                <><input ref={ownerIdRef} type="file" accept="image/*" capture="environment" onChange={e=>handlePhoto(e,"ownerid")} style={{display:"none"}}/><button onClick={()=>ownerIdRef.current?.click()} style={{...primaryBtn}}>📷 Photographier la Pièce d'Identité</button></>
              )}
            </div>
            <div style={card}>
              <label style={lbl}>Détails de la Visite</label>
              <label style={{...lbl,fontSize:9}}>{t.ownerFields?.visitDate||"Date de visite"}</label>
              <input type="date" value={p.visitDate||""} onChange={e=>updProp("visitDate",e.target.value)} style={inp}/>
              <label style={{...lbl,fontSize:9}}>{t.ownerFields?.askingPrice||"Prix demandé (GNF)"}</label>
              <input type="number" value={p.ownerAskingPrice||""} onChange={e=>updProp("ownerAskingPrice",e.target.value)} placeholder="0" style={inp}/>
              <label style={{...lbl,fontSize:9}}>{t.ownerFields?.negNotes||"Notes de négociation"}</label>
              <textarea value={p.negotiationNotes||""} onChange={e=>updProp("negotiationNotes",e.target.value)} placeholder="Points clés, motivation du vendeur, flexibilité..." style={{...inp,minHeight:70,resize:"vertical"}}/>
            </div>
          </>}

          {/* PHOTOS TAB */}
          {inspTab==="photos"&&<>
            <div style={{...card,border:`1px solid ${photos.length>=10?S.success+"44":S.warning+"44"}`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <div><label style={lbl}>{t.photoTitle}</label><div style={{fontSize:13,color:photos.length>=10?S.success:S.warning}}>{photos.length}/10 {t.photoMin} {photos.length>=10?"✓":""}</div></div>
                <span style={bdg(photos.length>=10?"#27ae60":"#e67e22")}>{photos.length}</span>
              </div>
              <div style={{background:S.border,borderRadius:3,height:5,marginBottom:10}}><div style={{background:photos.length>=10?"#27ae60":GOLD,height:5,borderRadius:3,width:`${Math.min(100,photos.length/10*100)}%`,transition:"width .3s"}}/></div>
              <input ref={photoRef} type="file" accept="image/*" multiple onChange={e=>handlePhoto(e,"main")} style={{display:"none"}}/>
              <div style={{display:"flex",gap:8}}>
                <button onClick={()=>{if(photoRef.current){photoRef.current.setAttribute("capture","environment");photoRef.current.click();}}} style={{...primaryBtn,flex:1,fontSize:14}}>{t.camera}</button>
                <button onClick={()=>{if(photoRef.current){photoRef.current.removeAttribute("capture");photoRef.current.click();}}} style={{flex:1,padding:"12px",borderRadius:8,border:`1.5px solid ${GOLD}44`,background:GOLD+"1a",color:GOLD,cursor:"pointer",fontSize:14,fontWeight:600,fontFamily:"inherit"}}>{t.uploadBtn}</button>
              </div>
            </div>
            <div style={{...card,background:dark?"#0d1117":"#f6f8fa"}}>
              <label style={{...lbl,marginBottom:8}}>Checklist des prises</label>
              {t.photoChecklist.map((item,i)=><div key={i} style={{display:"flex",gap:10,alignItems:"center",padding:"5px 0",borderBottom:`1px solid ${S.border}`}}><span style={{color:photos.length>i?S.success:S.text2,fontSize:14}}>{photos.length>i?"✓":"○"}</span><span style={{fontSize:13,color:photos.length>i?S.text:S.text2}}>{item}</span></div>)}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:7}}>
              {photos.map((ph,i)=>(
                <div key={i} style={{position:"relative",borderRadius:8,overflow:"hidden",background:S.bg3,aspectRatio:"4/3",border:`1px solid ${S.border}`,cursor:"pointer"}} onClick={()=>setShowLightbox(i)}>
                  <img src={ph.dataUrl||ph.url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}} loading="lazy"/>
                  <div style={{position:"absolute",top:3,left:3,background:GOLD+"cc",color:"#000",fontSize:9,fontWeight:700,padding:"1px 5px",borderRadius:3}}>#{i+1}</div>
                  <button onClick={e=>{e.stopPropagation();setPhotos(prev=>prev.filter((_,j)=>j!==i));}} style={{position:"absolute",top:3,right:3,background:"#c0392bcc",color:"#fff",border:"none",borderRadius:"50%",width:19,height:19,cursor:"pointer",fontSize:10}}>✕</button>
                  {ph.type&&<div style={{position:"absolute",bottom:0,left:0,right:0,background:"#000a",color:"#fff",fontSize:9,padding:"1px 4px"}}>{ph.type}</div>}
                </div>
              ))}
            </div>
          </>}
          ...
          {inspTab==="docs"&&<>
            <div style={card}>
              <label style={lbl}>{t.docsTitle}</label>
              <div style={{display:"flex",gap:6,marginBottom:12}}>
                {[["land",lang==="fr"?"🌍 Terrain":"🌍 Land"],["house",lang==="fr"?"🏠 Maison":"🏠 House"]].map(([v,l2])=><button key={v} onClick={()=>setDocType(v)} style={{flex:1,padding:"8px",borderRadius:7,border:`1.5px solid ${docType===v?GOLD:S.border}`,background:docType===v?GOLD+"1a":"transparent",color:docType===v?GOLD:S.text2,fontSize:13,cursor:"pointer",fontWeight:700}}>{l2}</button>)}
              </div>
              {currentDocs.map(doc=>(
                <div key={doc.id} style={{marginBottom:12,paddingBottom:12,borderBottom:`1px solid ${S.border}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                    <div style={{flex:1,display:"flex",gap:9,alignItems:"flex-start"}}>
                      <button onClick={()=>toggleDoc(doc.id)} style={{width:22,height:22,borderRadius:5,border:`2px solid ${docs[doc.id]?.available?GOLD:S.border}`,background:docs[doc.id]?.available?GOLD:"transparent",color:"#0d1117",cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:2}}>{docs[doc.id]?.available?"✓":""}</button>
                      <div><div style={{fontSize:13,fontWeight:600,color:docs[doc.id]?.available?S.text:S.text2}}>{lang==="fr"?doc.label:doc.labelEn}</div><div style={{fontSize:11,color:S.text2}}>{doc.desc}</div></div>
                    </div>
                    {docs[doc.id]?.available&&(
                      <div style={{display:"flex",gap:5,alignItems:"center",flexShrink:0}}>
                        <span style={bdg("#27ae60")}>{(docPhotos[doc.id]||[]).length}</span>
                        <label htmlFor={`dp-${doc.id}`} style={{padding:"5px 10px",borderRadius:6,background:GOLD+"1a",color:GOLD,border:`1px solid ${GOLD}44`,cursor:"pointer",fontSize:11}}>📷</label>
                        <input id={`dp-${doc.id}`} type="file" accept="image/*,.pdf" capture="environment" onChange={e=>handlePhoto(e,"doc",doc.id)} style={{display:"none"}}/>
                      </div>
                    )}
                  </div>
                  {docs[doc.id]?.available&&(docPhotos[doc.id]||[]).length>0&&(
                    <div style={{display:"flex",gap:4,marginTop:8,flexWrap:"wrap"}}>
                      {(docPhotos[doc.id]||[]).map((ph,i)=><div key={i} style={{position:"relative"}}><img src={ph.dataUrl||ph.url} alt="" style={{width:54,height:54,objectFit:"cover",borderRadius:4,border:`1px solid ${S.border}`}} loading="lazy"/><button onClick={()=>setDocPhotos(prev=>({...prev,[doc.id]:prev[doc.id].filter((_,j)=>j!==i)}))} style={{position:"absolute",top:-4,right:-4,background:"#c0392b",color:"#fff",border:"none",borderRadius:"50%",width:16,height:16,cursor:"pointer",fontSize:9}}>✕</button></div>)}
                    </div>
                  )}
                </div>
              ))}
          ...
          {inspTab==="plan"&&<>
            ...
            {floorMode==="2d"&&<>
              <div style={{display:"flex",gap:4,marginBottom:10,overflowX:"auto"}}>{floorList.map((f,i)=><button key={i} onClick={()=>setFloorNum(i)} style={{...tabBtn(floorNum===i),padding:"5px 8px"}}>{f}</button>)}</div>
              <div style={card}><label style={lbl}>{floorList[floorNum]}</label><Canvas2D elements={floorPlans[floorNum]||[]} onChange={els=>setFloorPlans(prev=>({...prev,[floorNum]:els}))} dark={dark}/></div>
              <div style={{...card,background:dark?"#0d1117":"#f6f8fa"}}><label style={lbl}>Résumé des étages</label>{floorList.map((f,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:`1px solid ${S.border}`}}><span style={{fontSize:12}}>{f}</span><span style={{fontSize:11,color:(floorPlans[i]||[]).length>0?GOLD:S.text2}}>{(floorPlans[i]||[]).length>0?`${floorPlans[i].length} éléments`:"—"}</span></div>)}</div>
            </>}
            {floorMode==="3d"&&<div style={card}><label style={lbl}>Vue 3D Interactive</label><p style={{color:S.text2,fontSize:11,marginBottom:10}}>Glissez pour orbiter · Scroll pour zoomer</p><Canvas3D floorPlans={floorPlans} dark={dark}/></div>}
          </>}
          ...
          {inspTab==="land"&&<>
            <div style={card}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <label style={lbl}>{t.landMap}</label>
                <button onClick={()=>setShowCameraMeasure(true)} style={{padding:"6px 10px",borderRadius:7,border:`1px solid ${GOLD}44`,background:GOLD+"1a",color:GOLD,cursor:"pointer",fontSize:11}}>{t.measureBtn}</button>
              </div>
              <p style={{color:S.text2,fontSize:12,marginBottom:10}}>Appuyez pour marquer les coins. Marchez à chaque coin et enregistrez un point GPS pour une mesure précise.</p>
              <LandCanvas points={Array.isArray(landPts)?landPts:[]} onChange={setLandPts} gps={gps} dark={dark}/>
            </div>
            <div style={card}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <label style={lbl}>Points GPS des frontières</label>
                <button onClick={()=>{if(!gps){alert("GPS non disponible.");return;}const pts=Array.isArray(landPts)?landPts:[];setLandPts([...pts,{px:40+(pts.length*38)%240,py:55+Math.floor(pts.length*38/240)*45,lat:gps.lat,lng:gps.lng}]);}} style={{padding:"6px 12px",borderRadius:7,border:`1px solid ${GOLD}44`,background:GOLD+"1a",color:GOLD,cursor:"pointer",fontSize:11,fontWeight:700}}>{t.addGPS}</button>
              </div>
              {gps&&<div style={{background:dark?"#0d1117":"#f6f8fa",borderRadius:6,padding:"8px 10px",fontFamily:"monospace",fontSize:12,color:GOLD,marginBottom:8}}>{gps.lat.toFixed(7)}°N, {gps.lng.toFixed(7)}°E ±{gps.acc}m</div>}
              {Array.isArray(landPts)&&landPts.filter(p=>p.lat).length>0&&(()=>{const gpsPts=landPts.filter(p=>p.lat);const area=gpsPts.length>=3?gpsArea(gpsPts):null;return(<div>{gpsPts.map((pt,i)=><div key={i} style={{fontFamily:"monospace",fontSize:10,color:S.text2,padding:"2px 0"}}>P{i+1}: {pt.lat.toFixed(7)}, {pt.lng.toFixed(7)}</div>)}{area&&<div style={{background:dark?"#071a07":"#f0fff4",borderRadius:8,padding:"10px",marginTop:8,border:`1px solid ${dark?"#1b5e2033":"#c8e6c9"}`}}><div style={{color:dark?"#4ade80":"#2e7d32",fontSize:12,fontWeight:700,marginBottom:6}}>Calcul de surface GPS</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:5}}>{[["m²",area.sqM],["ft²",area.sqFt],["Acres",area.acres],["Ha",area.ha]].map(([l2,v])=><div key={l2} style={{textAlign:"center"}}><div style={{color:S.text2,fontSize:9}}>{l2}</div><div style={{color:GOLD,fontSize:13,fontWeight:700}}>{v}</div></div>)}</div></div>}<a href={`https://maps.google.com/?q=${gpsPts[0].lat},${gpsPts[0].lng}`} target="_blank" rel="noreferrer" style={{color:S.info,fontSize:12,display:"block",marginTop:8}}>Voir sur Google Maps →</a></div>);})()}
            </div>
          </>}
          ...
          {inspTab==="notes"&&<>
            <div style={card}>
              <label style={lbl}>Notes et Observations</label>
              <textarea value={p.notes||""} onChange={e=>updProp("notes",e.target.value)} placeholder="Détails supplémentaires, observations, recommandations..." style={{...inp,minHeight:120,resize:"vertical"}}/>
            </div>
            <div style={card}>
              <label style={lbl}>Notes Vocales</label>
              <p style={{color:S.text2,fontSize:12,marginBottom:10}}>Maintenez le bouton pour enregistrer. Relâchez pour envoyer.</p>
              <VoiceRecorder onSend={handleVoiceNote} dark={dark}/>
              {voiceNotes.length>0&&<div style={{marginTop:12,display:"flex",flexDirection:"column",gap:8}}>{voiceNotes.map((vn,i)=><div key={i} style={{background:dark?"#0d1117":"#f6f8fa",borderRadius:8,padding:"8px 12px",border:`1px solid ${S.border}`}}><div style={{color:S.text2,fontSize:10,marginBottom:6}}>{fmtTime(vn.ts)} · {vn.agentName}</div><AudioPlayer url={vn.url} dark={dark}/></div>)}</div>}
            </div>
          </>}
  ...
  if(view==="admin"){
    ...
    return(
      <div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",background:S.bg,fontFamily:"system-ui,sans-serif",paddingBottom:70}}>
        <style>{CSS}</style>
        {offline&&<div style={{background:S.warning+"22",padding:"6px 16px",fontSize:12,color:S.warning,textAlign:"center"}}>{t.offlineBanner}</div>}
        {/* Header */}
        <div style={{background:dark?"#0d1117":"#fff",borderBottom:`1px solid ${GOLD}33`,padding:"9px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:10}}>
          <div><div style={{color:GOLD,fontWeight:700,fontSize:12,letterSpacing:2}}>{t.appName}</div><div style={{color:S.text2,fontSize:9,letterSpacing:0.8}}>{t.admin}</div></div>
          <div style={{display:"flex",gap:5,flexWrap:"wrap",justifyContent:"flex-end"}}>
            {pending.length>0&&<span style={{...bdg("#e67e22"),fontSize:9}}>⏳ {pending.length}</span>}
            <button onClick={()=>exportCSV(allInspections)} style={{padding:"5px 8px",borderRadius:6,border:`1px solid #27ae6044`,background:"#27ae6022",color:"#27ae60",cursor:"pointer",fontSize:10}}>{t.downloadCSV}</button>
            <button onClick={()=>setView("chat")} style={{padding:"5px 8px",borderRadius:6,border:`1px solid ${GOLD}44`,background:GOLD+"1a",color:GOLD,cursor:"pointer",fontSize:10}}>{t.chat}</button>
            <button onClick={()=>setLang(lang==="fr"?"en":"fr")} style={{...secondaryBtn(),padding:"4px 7px",fontSize:10}}>🌐</button>
            <button onClick={()=>setDark(!dark)} style={{...secondaryBtn(),padding:"4px 7px",fontSize:10}}>{dark?"☀":"🌙"}</button>
            <button onClick={doLogout} style={{...secondaryBtn(),padding:"4px 7px",fontSize:10}}>{t.logout}</button>
          </div>
        </div>
        {/* Stats */}
        <div style={{display:"flex",gap:6,padding:"10px 14px"}}>
          {[[allInspections.length,"Total",GOLD],[submittedInsp.length,t.submitted_b,"#27ae60"],[allInspections.length-submittedInsp.length,t.draft_b,"#e67e22"],[users.filter(u=>u.role==="agent").length,"Agents","#58a6ff"]].map(([n,l2,c])=>(
            <div key={l2} style={{flex:1,background:S.bg3,borderRadius:9,padding:"8px 4px",textAlign:"center",border:`1px solid ${c}22`}}>
              <div style={{color:c,fontSize:20,fontWeight:700}}>{n}</div>
              <div style={{color:S.text2,fontSize:9}}>{l2}</div>
            </div>
          ))}
        </div>
        {/* Admin tabs */}
        <div style={{display:"flex",gap:4,padding:"0 14px",marginBottom:10,overflowX:"auto"}}>
          {[["live",t.liveTitle],["submissions",t.submissions],["tracking",t.tracking],["team",t.team]].map(([id,l2])=>(
            <button key={id} onClick={()=>setAdminTab(id)} style={{...tabBtn(adminTab===id),whiteSpace:"nowrap"}}>{l2}</button>
          ))}
        </div>
        <div style={{padding:"0 14px",paddingBottom:30}}>

          {/* LIVE FEED */}
          {adminTab==="live"&&<>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><label style={lbl}>{t.liveTitle}</label><span style={bdg("#27ae60")}>● Live 4s</span></div>
            {allInspections.length===0&&<div style={{color:S.text2,textAlign:"center",padding:40}}>En attente d'activité des agents...</div>}
            {allInspections.map(ins=>(
              <div key={ins.id} style={card}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  <div>
                    <div style={{display:"flex",gap:7,alignItems:"center",marginBottom:3}}>
                      <div style={{width:28,height:28,borderRadius:"50%",background:GOLD+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:GOLD}}>{ins.agentName?.slice(0,2).toUpperCase()}</div>
                      <span style={{fontSize:13,fontWeight:600}}>{ins.agentName}</span>
                    </div>
                    <div style={{color:S.text,fontSize:12}}>{ins.property?.address||"Sans adresse"}</div>
                    <div style={{color:S.text2,fontSize:11}}>{ins.property?.region||""}{ins.property?.commune?" › "+ins.property.commune:""}</div>
                    <div style={{color:S.text2,fontSize:11,marginTop:3}}>📸{ins.photos?.length||0} · 📄{Object.keys(ins.documents||{}).filter(d=>ins.documents[d]?.available).length}{ins.ownerIdPhoto?" · 👤✓":""}{ins.location?" · 📍✓":""}</div>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <span style={bdg(ins.status==="submitted"?"#27ae60":"#e67e22")}>{ins.status==="submitted"?t.submitted_b:t.draft_b}</span>
                    <div style={{color:S.text2,fontSize:9,marginTop:3}}>{fmt(ins.lastSaved||ins.timestamp)}</div>
          ...
          {adminTab==="submissions"&&<>
            {allInspections.map(ins=>(
              <div key={ins.id} style={card}>
                <div onClick={()=>setExpandedInsp(expandedInsp===ins.id?null:ins.id)} style={{cursor:"pointer"}}>
                  <div style={{display:"flex",justifyContent:"space-between"}}>
                    <div style={{flex:1}}>
                      <div style={{fontWeight:600,fontSize:13}}>{ins.property?.address||"Sans adresse"}</div>
                      <div style={{color:S.text2,fontSize:11}}>Par <span style={{color:GOLD}}>{ins.agentName}</span> · {fmt(ins.timestamp)}</div>
                      <div style={{display:"flex",gap:7,marginTop:3}}>
                        <span style={{color:S.text2,fontSize:11}}>🛏{ins.property?.bedrooms||0}</span>
                        <span style={{color:S.text2,fontSize:11}}>📸{ins.photos?.length||0}</span>
                        {ins.ownerIdPhoto&&<span style={{color:GOLD,fontSize:11}}>👤✓</span>}
                        {ins.location&&<span style={{color:"#27ae60",fontSize:11}}>📍✓</span>}
                      </div>
                    </div>
                    <div style={{display:"flex",flexDirection:"column",gap:4,alignItems:"flex-end"}}>
                      <span style={bdg(ins.status==="submitted"?"#27ae60":"#e67e22")}>{ins.status==="submitted"?t.submitted_b:t.draft_b}</span>
                      <button onClick={e=>{e.stopPropagation();delInspection(ins.id);}} style={{padding:"2px 7px",borderRadius:5,border:`1px solid ${S.error}44`,background:"transparent",color:S.error,cursor:"pointer",fontSize:10}}>Supprimer</button>
                    </div>
                  </div>
                </div>
                {expandedInsp===ins.id&&(
                  <div style={{marginTop:12,paddingTop:12,borderTop:`1px solid ${S.border}`}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5,marginBottom:10}}>
                      {[["Type",ins.property?.type],["État",ins.property?.condition],["Chambres",ins.property?.bedrooms],["SdB",ins.property?.bathrooms],["Salons",ins.property?.livingRooms],["Surface",ins.property?.totalArea?ins.property.totalArea+"m²":"—"],["Terrain",ins.property?.landHectares?ins.property.landHectares+" ha":"—"],["Prix",ins.property?.listPrice?ins.property.listPrice+" GNF":"—"]].map(([l2,v])=>(
                        <div key={l2} style={{background:dark?"#0d1117":"#f6f8fa",borderRadius:5,padding:"5px 8px"}}>
                          <div style={{color:S.text2,fontSize:9,textTransform:"uppercase"}}>{l2}</div>
                          <div style={{color:S.text,fontSize:12,fontWeight:600,textTransform:"capitalize"}}>{v||"—"}</div>
                        </div>
                      ))}
                    </div>
                    {ins.property?.ownerName&&<div style={{background:dark?"#0d1117":"#f6f8fa",borderRadius:6,padding:"8px",marginBottom:8}}><div style={{color:S.text2,fontSize:9}}>PROPRIÉTAIRE</div><div style={{color:S.text,fontSize:13}}>{ins.property.ownerFirstName} {ins.property.ownerName}</div>{ins.property.ownerPhone&&<div style={{color:S.text2,fontSize:11}}>{ins.property.ownerPhone}</div>}</div>}
                    {ins.property?.features?.length>0&&<div style={{background:dark?"#0d1117":"#f6f8fa",borderRadius:6,padding:"8px",marginBottom:8}}><div style={{color:S.text2,fontSize:9}}>ÉQUIPEMENTS</div><div style={{color:S.text,fontSize:12}}>{ins.property.features.join(" · ")}</div></div>}
                    {ins.location&&<div style={{background:dark?"#0d1117":"#f6f8fa",borderRadius:6,padding:"8px",marginBottom:8}}><div style={{color:S.text2,fontSize:9}}>GPS</div><div style={{color:GOLD,fontFamily:"monospace",fontSize:12}}>{ins.location.lat.toFixed(7)}°N, {ins.location.lng.toFixed(7)}°E</div><a href={`https://maps.google.com/?q=${ins.location.lat},${ins.location.lng}`} target="_blank" rel="noreferrer" style={{color:S.info,fontSize:11}}>Google Maps →</a></div>}
                    {ins.ownerIdPhoto&&<div style={{marginBottom:8}}><div style={{color:S.text2,fontSize:9,marginBottom:4}}>PIÈCE D'IDENTITÉ DU PROPRIÉTAIRE</div><img src={ins.ownerIdPhoto.dataUrl||ins.ownerIdPhoto.url} alt="ID" style={{width:"100%",borderRadius:6,border:`1px solid ${S.border}`}} loading="lazy"/></div>}
                    {ins.photos?.length>0&&<div><div style={{color:S.text2,fontSize:9,marginBottom:5}}>PHOTOS ({ins.photos.length})</div><div style={{display:"flex",gap:4,overflowX:"auto",paddingBottom:4}}>{ins.photos.map((ph,i)=><img key={i} src={ph.dataUrl||ph.url} alt="" style={{width:62,height:62,objectFit:"cover",borderRadius:4,flexShrink:0,border:`1px solid ${S.border}`,cursor:"pointer"}} loading="lazy" onClick={()=>{const a=document.createElement("a");a.href=ph.dataUrl||ph.url;a.download=`photo_${i+1}.jpg`;a.click();}}/>)}</div></div>}
                    {ins.property?.notes&&<div style={{background:dark?"#0d1117":"#f6f8fa",borderRadius:6,padding:"8px",marginTop:8}}><div style={{color:S.text2,fontSize:9}}>NOTES AGENT</div><div style={{color:S.text2,fontSize:12,lineHeight:1.5}}>{ins.property.notes}</div></div>}
                    {ins.voiceNotes?.length>0&&<div style={{marginTop:8}}><div style={{color:S.text2,fontSize:9,marginBottom:6}}>NOTES VOCALES ({ins.voiceNotes.length})</div>{ins.voiceNotes.map((vn,i)=><div key={i} style={{background:dark?"#0d1117":"#f6f8fa",borderRadius:8,padding:"8px",marginBottom:4,border:`1px solid ${S.border}`}}><div style={{color:S.text2,fontSize:10,marginBottom:4}}>{vn.agentName} · {fmtTime(vn.ts)}</div><AudioPlayer url={vn.url} dark={dark}/></div>)}</div>}
                  </div>
                )}
              </div>
            ))}
            {allInspections.length===0&&<div style={{color:S.text2,textAlign:"center",padding:40}}>Aucune inspection.</div>}
          </>}

          {/* TRACKING */}
          {adminTab==="tracking"&&<>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><label style={lbl}>{t.tracking}</label><span style={bdg("#27ae60")}>● 30s</span></div>
            {Object.values(agentLocs).length===0&&<div style={{color:S.text2,textAlign:"center",padding:30}}>Aucune localisation. Les agents apparaissent ici dès leur connexion.</div>}
            {Object.values(agentLocs).map((loc,i)=>(
              <div key={i} style={card}>
                <div style={{display:"flex",justifyContent:"space-between"}}>
                  <div>
                    <div style={{fontWeight:600,fontSize:13,marginBottom:3}}>{loc.name}</div>
                    <div style={{color:GOLD,fontFamily:"monospace",fontSize:11}}>{loc.lat?.toFixed(6)}°N, {loc.lng?.toFixed(6)}°E</div>
                    <div style={{color:S.text2,fontSize:10,marginTop:2}}>Vu: {fmt(loc.ts)} · ±{loc.acc}m</div>
                  </div>
                  <div style={{display:"flex",flexDirection:"column",gap:6,alignItems:"flex-end"}}>
                    <span style={bdg(Date.now()-loc.ts<600000?"#27ae60":"#e74c3c")}>{Date.now()-loc.ts<600000?t.active_b:t.suspended_b}</span>
                    <a href={`https://maps.google.com/?q=${loc.lat},${loc.lng}`} target="_blank" rel="noreferrer" style={{color:S.info,fontSize:11}}>Suivre →</a>
                  </div>
                </div>
              </div>
            ))}
          ...
          {/* TEAM */}
          {adminTab==="team"&&<>
            {pending.length>0&&<div style={{...card,border:`1px solid ${S.warning}44`}}>
              <label style={{...lbl,color:S.warning}}>⏳ En attente d'approbation ({pending.length})</label>
              {pending.map(u=>(
                <div key={u.uid} style={rowStyle}>
                  <div><div style={{fontSize:13,fontWeight:600}}>{u.displayName}</div><div style={{color:S.text2,fontSize:11}}>{u.email}</div></div>
                  <div style={{display:"flex",gap:6}}>
                    <button onClick={()=>approveUser(u.uid)} style={{padding:"6px 12px",borderRadius:6,background:"#27ae60",color:"#fff",border:"none",cursor:"pointer",fontSize:12,fontWeight:700}}>{t.approve}</button>
                    <button onClick={()=>deleteUserDB(u.uid)} style={{padding:"6px 10px",borderRadius:6,border:`1px solid ${S.error}44`,background:"transparent",color:S.error,cursor:"pointer",fontSize:12}}>{t.deleteUser}</button>
                  </div>
                </div>
              ))}
            </div>}
            <div style={{...card,background:dark?"#0d1117":"#f6f8fa"}}>
              <label style={lbl}>Tous les membres ({users.length})</label>
              {users.map(u=>(
                <div key={u.uid} style={{...rowStyle,flexWrap:"wrap",gap:8}}>
                  <div style={{flex:1}}>
                    <div style={{fontSize:13,fontWeight:600}}>{u.displayName||u.email}</div>
                    <div style={{color:S.text2,fontSize:11}}>{u.email}</div>
                    <div style={{display:"flex",gap:5,marginTop:3}}>
                      <span style={bdg(u.role==="admin"?GOLD:"#58a6ff")}>{u.role}</span>
                      <span style={bdg(u.status==="active"?"#27ae60":u.status==="pending"?"#e67e22":"#e74c3c")}>{u.status==="active"?t.active_b:u.status==="pending"?t.pending_b:t.suspended_b}</span>
                    </div>
                  </div>
                  {u.uid!==fbUser?.uid&&(
                    <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                      {u.status==="active"&&<button onClick={()=>suspendUser(u.uid)} style={{padding:"5px 9px",borderRadius:6,border:`1px solid ${S.warning}44`,background:"transparent",color:S.warning,cursor:"pointer",fontSize:11}}>{t.suspend}</button>}
                      {u.status==="suspended"&&<button onClick={()=>unsuspendUser(u.uid)} style={{padding:"5px 9px",borderRadius:6,border:`1px solid #27ae6044`,background:"transparent",color:"#27ae60",cursor:"pointer",fontSize:11}}>{t.unsuspend}</button>}
                      {u.status==="pending"&&<button onClick={()=>approveUser(u.uid)} style={{padding:"5px 9px",borderRadius:6,background:"#27ae60",color:"#fff",border:"none",cursor:"pointer",fontSize:11,fontWeight:700}}>{t.approve}</button>}
                      <button onClick={()=>deleteUserDB(u.uid)} style={{padding:"5px 9px",borderRadius:6,border:`1px solid ${S.error}44`,background:"transparent",color:S.error,cursor:"pointer",fontSize:11}}>{t.deleteUser}</button>
                    </div>
                  )}
                </div>
              ))}
            ...
            {/* Change team PIN */}
            <div style={card}>
              <label style={lbl}>PIN d'équipe pour les nouvelles inscriptions</label>
              <p style={{color:S.text2,fontSize:12,marginBottom:10}}>Les nouveaux agents doivent saisir ce PIN lors de l'inscription.</p>
              <input placeholder="Nouveau PIN (4 chiffres)" maxLength={4} inputMode="numeric" style={{...inp,letterSpacing:6,fontSize:20,textAlign:"center",marginBottom:8}} onChange={e=>{if(e.target.value.length===4){set(ref(db,"settings/teamPin"),e.target.value).then(()=>alert("PIN mis à jour: "+e.target.value));}}}/>
              <p style={{color:S.text2,fontSize:11}}>Saisissez le nouveau PIN (4 chiffres) — sauvegarde automatique.</p>
            </div>
          </>}
        </div>
        {/* Bottom Nav admin */}
        <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:dark?"#161b22":"#fff",borderTop:`1px solid ${S.border}`,display:"flex",justifyContent:"space-around",padding:"8px 0 12px",zIndex:20}}>
          {[["Direct","🔴","live"],["Soumissions","📋","submissions"],["GPS","📍","tracking"],["Équipe","👥","team"]].map(([label,icon,tab])=>(
            <button key={label} onClick={()=>setAdminTab(tab)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,background:"none",border:"none",cursor:"pointer",color:adminTab===tab?S.gold:S.text2,fontSize:10,minWidth:52}}>
              <span style={{fontSize:20}}>{icon}</span>
              {label}
              {adminTab===tab&&<div style={{width:4,height:4,borderRadius:"50%",background:GOLD}}/>}
            </button>
          ))}
        </div>
      </div>
  ...
  return <div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",background:S.bg,padding:20,color:S.text2,fontFamily:"system-ui,sans-serif"}}><style>{CSS}</style>Chargement...</div>;
}
